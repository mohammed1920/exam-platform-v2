...
els.section.querySelectorAll('[data-lawyer-view]').forEach(btn=>btn.addEventListener('click',()=>switchLawyerView(btn.dataset.lawyerView)));
  ...
  function openPanel(){if(!isAdmin||!els.section)return;panelOpen=true;els.section.hidden=false;els.section.classList.add('active');els.section.scrollIntoView({behavior:'smooth',block:'start'});loadDashboard();}
  function closePanel(){if(!panelOpen)return;els.section.hidden=true;els.section.classList.remove('active');panelOpen=false;}
