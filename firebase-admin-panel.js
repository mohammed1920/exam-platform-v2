...
(function () {
  ...
  function buildUI(){
    ...
    section.innerHTML=`<div class="firebase-admin-head"><div><div class="firebase-admin-kicker">⚖️ لوحة الإدارة</div><h2>لوحة إدارة المنصة</h2><p id="firebase-admin-welcome">تم التحقق من صلاحية الإدارة.</p></div><button id="firebase-admin-refresh" class="firebase-admin-action" type="button">↻ تحديث البيانات</button></div><div id="firebase-admin-status" class="firebase-admin-status" hidden></div><div class="firebase-admin-stats"><div class="firebase-admin-stat"><span>إجمالي الطلاب</span><strong id="fa-students-count">—</strong></div><div class="firebase-admin-stat"><span>النتائج المسجلة</span><strong id="fa-results-count">—</strong></div><div class="firebase-admin-stat"><span>طلاب لديهم نتائج</span><strong id="fa-active-count">—</strong></div><div class="firebase-admin-stat"><span>متوسط النسبة</span><strong id="fa-average-score">—</strong></div></div><div class="firebase-admin-menu"><button type="button" class="firebase-admin-menu-card active" data-admin-view="students"><i class="fas fa-users"></i><strong>الطلاب المسجلون</strong><span>عرض حسابات الطلاب المسجلين</span></button><button type="button" class="firebase-admin-menu-card" data-admin-view="lawyers"><i class="fas fa-user-tie"></i><strong>إدارة دليل المحامين</strong><span>الطلبات والملفات المنشورة</span></button><button type="button" class="firebase-admin-menu-card" data-admin-view="results"><i class="fas fa-chart-column"></i><strong>آخر النتائج</strong><span>متابعة نتائج الاختبارات الأخيرة</span></button></div><div class="firebase-admin-subviews"><section id="fa-admin-view-students" class="firebase-admin-subview active"><div class="firebase-admin-card"><div class="firebase-admin-card-head"><h3>👥 الطلاب المسجلون</h3><span id="fa-students-updated"></span></div><div class="firebase-admin-table-wrap"><table><thead><tr><th>الاسم</th><th>البريد</th><th>التسجيل</th></tr></thead><tbody id="fa-students-body"><tr><td colspan="3">جارٍ التحميل...</td></tr></tbody></table></div></div></section><section id="fa-admin-view-results" class="firebase-admin-subview"><div class="firebase-admin-card"><div class="firebase-admin-card-head"><h3>📝 آخر النتائج</h3><span id="fa-results-updated"></span></div><div class="firebase-admin-table-wrap"><table><thead><tr><th>الطالب</th><th>الكتاب</th><th>الفصل</th><th>النتيجة</th></tr></thead><tbody id="fa-results-body"><tr><td colspan="4">جارٍ التحميل...</td></tr></tbody></table></div></div></section><section id="fa-admin-view-lawyers" class="firebase-admin-subview"><div class="firebase-admin-card lawyer-admin-card">
...
els.section.querySelectorAll('[data-lawyer-view]').forEach(btn=>btn.addEventListener('click',()=>switchLawyerView(btn.dataset.lawyerView)));
  ...
  function switchAdminView(view){
    els.section.querySelectorAll('[data-admin-view]').forEach(btn=>btn.classList.toggle('active',btn.dataset.adminView===view));
