...
(function () {
  ...
  function scoreFromResults(results, book) {
    ...
    results.forEach(doc => {
      ...
      if (!current || score / total > current.score / current.total ||
        ...
        bestByChapter[chapter] = { score, total, duration: Number(data.duration) || 0 };
      }
    });

    const chapters = Object.keys(bestByChapter).map(Number).sort((a, b) => a - b);
  ...
  async function syncBook(book) {
    ...
    const snapshot = await firestore.collection('examResults')
    ...
    // Each student can only write their own entry. Server-side aggregation will replace
    // this client calculation later when the online API/Cloud Function layer is enabled.
    await firestore.collection('leaderboards').doc(book.id)
      .collection('entries').doc(currentUser.uid).set(profile, { merge: true });
    return profile;
  }
  ...
  async function loadTop20(book) {
    ...
    const snap = await firestore.collection('leaderboards').doc(book.id)
      .collection('entries')
      .where('eligible', '==', true)
  ...
  function renderTable(entries, own, book) {
    if (!entries.length) {
      return `<div class="profile-panel leaderboard-empty"><div class="leaderboard-empty-icon">🏆</div><h3>لم تبدأ المنافسة بعد</h3><p>أكمل 3 فصول على الأقل لتدخل الترتيب الرسمي.</p></div>`;
    }
    const rows = entries.map((entry, index) => {
      const rank = index + 1;
      const isMe = own && entry.uid === own.uid;
      const name = isMe ? 'أنت' : (entry.displayName || 'طالب');
      return `<article class="leaderboard-row ${isMe ? 'is-me' : ''}">
        <div class="leaderboard-rank">${medal(rank)}</div>
        <div class="leaderboard-student"><strong>${esc(name)}</strong><small>${Number(entry.completedChapters) || 0} فصول مكتملة · ${Number(entry.weightedSuccess * 100 || 0).toFixed(1)}%</small></div>
        <div class="leaderboard-score"><strong>${Number(entry.finalScore || 0).toFixed(1)}</strong><small>/ 1000</small></div>
      </article>`;
    }).join('');
  ...
  async function render(bookId) {
    ...
    renderLoading(root);
    try {
      // Sync only the current student's entry, then read only the top 20 public entries.
  ...
  function injectMenu() {
    ...
    button.type = 'button';
    button.className = 'student-sidebar-item';
    button.dataset.sidebarTarget = 'leaderboard';
    ...
    button.addEventListener('click', () => {
      ...
      render(bookIdFromState());
    });
  }

  function bookIdFromState() {
    const state = window.history && window.history.state;
    return state && state.bookId ? state.bookId : (books()[0] && books()[0].id);
  }
  ...
  function install() {
    ...
    const wait = setInterval(() => {
      injectMenu();
      if (document.getElementById('student-account-sidebar')) clearInterval(wait);
    }, 100);
    setTimeout(() => clearInterval(wait), 10000);

    window.addEventListener('public-auth-state-changed', () => injectMenu());
    window.addEventListener('student-history-updated', () => {});
    window.addEventListener('firestore-exam-result-saved', event => {
      const bookId = event.detail && event.detail.bookId;
      if (!bookId || !user()) return;
      const book = bookById(bookId);
      if (book) syncBook(book).catch(error => console.warn('Leaderboard sync failed:', error));
    });

    // Expose a small public API for the account UI.
